...
// Debug fallback for unknown manager routes.
// It does NOT automatically authorize users; it only returns route info so you can see what the extension called.
app.all("/api/manager/*", (req, res) => {
  console.log("Unknown manager route:", req.method, req.originalUrl, { body: req.body, query: req.query });
  res.status(404).json({
    ok: false,
    registered: false,
    error: "Manager route not implemented yet",
    method: req.method,
    path: req.path,
    hint: "Send this path to ChatGPT so it can be added to the PUREX STORE API."
  });
});
